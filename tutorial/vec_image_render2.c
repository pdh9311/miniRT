...
double	cross(t_vec v1, t_vec v2)
{
...
void close(t_data *data)	// 이벤트 발생시 call될 함수
{
	 ...
	 * free(data->mlx);
	 */
	system("leaks vec_image_render2");	// memory leaks check
	exit(EXIT_SUCCESS);
}
...
/*
	t_point3	origin = vec(0, 0, 0);
	t_point3	focal = vec_add(origin, vec(0, 0, -focal_length));
	t_point3	horizontal = vec_add(focal, vec(-(viewport_width / 2), 0, 0));
	t_point3	vertical = vec_add(focal, vec(0, -(viewport_height / 2), 0));

	t_vec	viewport_center = vec_sub(focal, origin);
	t_vec	lower_left_corner = vec(
		viewport_center.x + horizontal.x + vertical.x,
		viewport_center.y + horizontal.y + vertical.y,
		viewport_center.z + horizontal.z + vertical.z
	);
*/
...
// 	t_vec	a = vec(1.0, 1.0, 1.0);
// 	t_vec	b = vec(0.5, 0.7, 1.0);
// 	return (vec_add(vec_mul_t(a, 1.0 - t), vec_mul_t(b, t)));
// }
